@extends('layouts.app')
@section('title', 'Hutang Kost')
@section('page-title', 'Hutang Kost')
...
@section('topbar-actions')
    ...
    <button class="btn btn-success" onclick="openModal('addGlobalRepaymentModal')">
        <i class="bi bi-cash"></i> Tambah Pembayaran
    </button>
@endsection
...
<div style="display:grid; grid-template-columns:repeat(4,1fr); gap:16px; margin-bottom:20px;">
    <div class="stat-card" style="--accent-color:#3b82f6;">
        ...
        <div><div class="stat-label">Total Hutang</div>
    ...
    <div class="stat-card" style="--accent-color:#a855f7;">
        ...
        <div>
            <div class="stat-label">Total Nominal Hutang</div>
...
<div class="card">
    <div class="card-header">
        <div class="card-title"><i class="bi bi-table"></i> Data Hutang ({{ $loans->total() }})</div>
    </div>
    @if($loans->count())
                ...
                <tr>
                    ...
                    <td>
                        <div class="flex gap-2">
                            <a href="{{ route('payables.show', $loan) }}" class="btn btn-info btn-sm btn-icon"><i class="bi bi-eye"></i></a>
                            ...
                            <form action="{{ route('payables.destroy', $loan) }}" method="POST" data-confirm="Hapus data hutang ini? Semua riwayat pembayaran terkait juga akan terhapus.">
                                @csrf @method('DELETE')
...
<div class="card mt-4" style="border-color:rgba(59,130,246,0.3); background:rgba(59,130,246,0.02);">
    <div class="card-header" style="display:flex; justify-content:space-between; align-items:center;">
        <div class="card-title text-primary"><i class="bi bi-link-45deg"></i> Pembayaran Bebas (Belum Ditautkan)</div>
    </div>
    <div class="table-wrapper">
        <table>
            <thead>
                ...
                <tr>
                    ...
                    <td>
                        <div class="flex gap-2">
                            ...
                            <form action="{{ route('payable-repayments.destroy', $rep) }}" method="POST" data-confirm="Hapus data pembayaran bebas ini?">
                                @csrf @method('DELETE')
                                <button type="submit" class="btn btn-danger btn-sm btn-icon"><i class="bi bi-trash"></i></button>
                            </form>
                        </div>
...
<div id="addLoanModal" class="modal-backdrop d-none">
    <div class="modal-card">
        <div class="card-header">
            <div class="card-title">Tambah Hutang Baru</div>
        ...
        <form action="{{ route('payables.store') }}" method="POST">
            ...
            <div class="form-group mb-4">
                <label>Nama Pemberi Pinjaman <span class="required">*</span></label>
                <input type="text" name="name" class="form-control" required>
            </div>
            <div class="form-group mb-4">
                <label>Keperluan Hutang <span class="required">*</span></label>
                <input type="text" name="purpose" class="form-control" required>
            </div>
            <div class="form-group mb-4">
                <label>Tanggal Hutang <span class="required">*</span></label>
                <input type="date" name="loan_date" class="form-control" value="{{ date('Y-m-d') }}" required>
            </div>
            <div class="form-group mb-4">
                <label>Total Hutang (Rp) <span class="required">*</span></label>
...
<div id="editLoanModal" class="modal-backdrop d-none">
    <div class="modal-card">
        <div class="card-header">
            <div class="card-title">Edit Hutang</div>
    ...
    <div class="modal-card">
        ...
        <form action="{{ route('payable-repayments.store') }}" method="POST">
            ...
            <div class="form-group mb-4">
                ...
                <span class="form-hint">Anda bisa menyimpan catatan uang keluar ini tanpa menautkannya ke hutang mana pun terlebih dahulu.</span>
            </div>
            <div class="form-group mb-4">
...
<div id="linkRepaymentModal" class="modal-backdrop d-none">
    <div class="modal-card">
        <div class="card-header">
            <div class="card-title">Tautkan Pembayaran</div>
        ...
        <form id="linkRepaymentForm" method="POST">
            ...
            <div style="background:rgba(59,130,246,0.05); padding:12px; border-radius:8px; margin-bottom:16px;">
                <div class="text-xs text-muted mb-1">Nominal Uang Keluar:</div>
                <div class="money-text text-danger" style="font-size:18px;" id="linkRepaymentAmountDisplay">Rp 0</div>
            </div>
            <div class="form-group mb-6">
                <label>Pilih Hutang <span class="required">*</span></label>
...
function openEditLoanModal(btn) {
    ...
    form.action = `/payables/${btn.dataset.id}`;
...
function openLinkRepaymentModal(id, amount) {
    ...
    form.action = `/payable-repayments/${id}/link`;
