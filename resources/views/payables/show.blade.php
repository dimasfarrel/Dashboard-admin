...
<div class="flex" style="gap:24px; flex-wrap:wrap;">
    ...
    <div class="card" style="flex:1; min-width:300px;">
        <div class="card-header" style="display:flex; justify-content:space-between; align-items:center;">
            <div class="card-title"><i class="bi bi-info-circle"></i> Informasi Hutang</div>
            <a href="{{ route('payables.index') }}" class="btn btn-secondary btn-sm">Kembali</a>
        </div>
        <div style="display:grid; grid-template-columns:140px 1fr; gap:12px 16px; font-size:14px;">
            <div class="text-muted">Nama Pemberi</div>
    ...
    <div class="card" style="flex:2; min-width:400px;">
        <div class="card-header" style="display:flex; justify-content:space-between; align-items:center;">
            ...
            @if(!$loan->is_paid)
            <button class="btn btn-success btn-sm" onclick="openModal('addRepaymentModal')"><i class="bi bi-plus"></i> Catat Pembayaran Keluar</button>
            @endif
        </div>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $rep->notes ?: '-' }}</td>
                        <td>
                            <form action="{{ route('payable-repayments.destroy', $rep) }}" method="POST" data-confirm="Hapus riwayat pembayaran ini?">
                                @csrf @method('DELETE')
        ...
        <div class="empty-state">
            ...
            <p>Belum ada riwayat pembayaran untuk hutang ini.</p>
        </div>
        @endif
    </div>
</div>
...
<div id="addRepaymentModal" class="modal-backdrop d-none">
    <div class="modal-card">
        ...
        <form action="{{ route('payables.repayments.store', $loan) }}" method="POST">
            @csrf
            ...
            <div class="form-group mb-4">
                ...
                <span class="form-hint">Sisa tagihan hutang: Rp {{ number_format($loan->remaining_amount, 0, ',', '.') }}</span>
            </div>
