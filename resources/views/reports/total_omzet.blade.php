...
@php
    ...
    $sumDeposit = $incomes->sum('deposit_amount');
    $sumPemasukan = $sumOmzet + $sumHutang + $sumDeposit;
@endphp
...
<div class="summary-cards no-print">
    ...
    <div class="summary-card">
        ...
        <div class="summary-content">
            ...
            <div class="summary-value">Rp {{ number_format($sumDeposit, 0, ',', '.') }}</div>
        </div>
    </div>
    <div class="summary-card">
        ...
        <table class="table">
            <thead>
                <tr style="background-color: #f8fafc;">
                    ...
                    <th style="text-align: right; width: 140px;">Nominal Deposit</th>
                    <th style="text-align: right; width: 140px; background-color: #fdefecff;">Total Masuk</th>
                </tr>
            </thead>
            <tbody>
                @forelse($incomes as $inc)
                    @php $rowTotal = $inc['omzet_amount'] + $inc['hutang_amount'] + $inc['deposit_amount']; @endphp
                    <tr>
                        ...
                        <td style="text-align: right; color: #0ea5e9;">{{ $inc['deposit_amount'] > 0 ? 'Rp ' . number_format($inc['deposit_amount'], 0, ',', '.') : '-' }}</td>
                        <td style="text-align: right; font-weight: bold; background-color: rgba(16, 185, 129, 0.05);">Rp {{ number_format($rowTotal, 0, ',', '.') }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7" style="text-align: center; padding: 30px; color: #64748b;">Belum ada data pemasukan pada periode ini.</td>
                    </tr>
                @endforelse
            </tbody>
            <tfoot>
                <tr style="font-weight: bold; background-color: #ecfdf5;">
                    ...
                    <td style="text-align: right; color: #0ea5e9;">Rp {{ number_format($sumDeposit, 0, ',', '.') }}</td>
